...
func (ls *LockServer) applyToStateMachine(op *Op) {
	...
	lockOwner, flag1 := ls.store.getLockOwner(op.Key);

	// Check which opertaion it is
	if op.Type == LockOp {
		ls.store.lock(op.Key, op.ClientId) // Lock 

		if ls.isLeader() {
			// Send to channel
			e := Event{Op: op.Type, key: op.Key, id: op.ClientId, eventType: ServerEvent}
			ls.events <- e 
		}
	} else if op.Type == UnlockOp {
		ls.store.unlock(op.Key, op.ClientId) // Lock 
	...
	newLockOwner, flag2 := ls.store.getLockOwner(op.Key);

	if newLockOwner != lockOwner || (flag2 && !flag1) {
		ls.giveLockToOwner(op.Key, newLockOwner)
	}
